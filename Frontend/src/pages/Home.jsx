  ...
  const handleLocationAndData = async () => {
    ...
    try {
      ...
      if (!user.location?.city) {
        ...
        // Get city from reverse geocoding after updating
        ...
        cityKeyword = user.location.city;
      }
    } catch (err) {
      console.warn(" Not logged in or failed to get user info.");

      // Try to detect location and get city for guest users
      try {
        const { lat, lon } = await detectLocation();
        const geoRes = await axios.get(
          `https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lon}&format=json`
        ...
        cityKeyword =
          ...
          geoRes.data.address.state ||
          "";
        console.log("🌐 Guest city from reverse geocode:", cityKeyword);
      } catch (geoErr) {
        console.warn(" Location detection failed for guest:", geoErr.message);
        cityKeyword = "";
      }
    }
  ...
  const filteredFoods = foods
    .filter(
      (food) =>
        ...
        (!selectedRating || food.rating >= parseFloat(selectedRating)) &&
